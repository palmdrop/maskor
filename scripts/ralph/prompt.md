...
- "Tests require the dev server running on PORT 3001"
...
1. **Update the source specs.** For each path in `prd.json`'s `specs` field, append a single bullet to that spec's `**Shipped**:` list under the top header block. Format:

   ```
   - YYYY-MM-DD — <prd.json `description` field>. (plan: `scripts/ralph/archive/YYYY-MM-DD-<branchName without "ralph/" prefix>/`)
   ```

   Use today's date for both occurrences. This is the ONLY change you make to the spec — do not edit anything else in it.

2. Commit the spec update with message: `docs: mark <feature> shipped in source specs`.
