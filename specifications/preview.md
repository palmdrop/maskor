...
- 2026-06-19 — Sticky current-fragment indicator: the active fragment's key (from the scroll-spy) renders as a minimal, muted "you are here" label in the sticky toolbar via the shared `ActiveFragmentLabel`, shown regardless of the fragment-titles toggle so the writer always knows which fragment they are in. The same component is used by the import preview.
...
- 2026-06-11 — Inline editing now opens the full fragment editor as a **center-replacing overlay** (the assembled document unmounts; the nav sidebar stays), replacing the markdown-split triptych and the minimal `InlineFragmentEditor` (both removed). Double-click resolves the fragment via the anchor sentinels (kept), Previous/Next walk the assembled order, `⌘Esc` (or Close) saves then exits, and on close the document scrolls back to the top of the last-shown fragment. The editor owns its own fetch + save round-trip. (ADR 0013; plan: `references/plans/fragment-editor-focus-mode.md`)
...
- The Margin (comments) column while editing inline — suppressed in the overlay; available in the dedicated editor and in focus mode. (Metadata, swap-recovery, and extract/insert now come with the full editor used inline — no longer out of scope.)
