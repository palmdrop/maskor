...
- 2026-07-16 — Case-only rename reporting fix. A piece-1 override (or an automatic stripped-heading rename) that changes the original's key only in case is now reported via `originalKeyRenamedTo` and the `fragment:split` log entry — matching the storage layer, which treats a case-only key change as a real rename (file + Margin rename, `[[fragments/oldKey]]` link rewrite). Previously the `renamed` flag compared lowercased and silently dropped these. A later-piece override that reuses the renamed original's still-reserved old key now rejects with a specific message (it is held until the split completes — key-swapping between pieces is not possible in one split) rather than the generic "already exists". The split preview now returns the original's current key so the dialog reads "(original)" when the user edits piece 1 back to that exact key.
...
- A split never loses body prose: it is fully preserved across piece 1 + the new pieces. With the default heading-stripping, each stripped heading is relocated into its piece's key rather than dropped (piece 1 → the original's key; pieces 2…N → their derived keys). Two deliberate exceptions drop the heading text: (1) a heading that sanitizes to an empty key (symbols only) derives a fallback key, so its heading text is not retained — keep the heading in the body to preserve it verbatim (see `references/suggestions.md`); (2) a **user key override** on a heading-stripped piece (including a piece-1 override) — the key becomes the user's choice, so the stripped heading text lands nowhere. Both are user-visible in the preview. No source archive is written (unlike import — `specifications/import-pipeline.md`).
