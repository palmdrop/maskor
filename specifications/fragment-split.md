...
- Renaming the new pieces (2…N) in the preview before committing; the original (piece 1) keeps its key unless the heading is stripped and its body starts with one (then it is renamed to that heading)
...
- A **preview** of the resulting pieces (keys + excerpts + count) before committing; the new pieces' keys are **editable** in the preview (piece 1 is the original — read-only, and flagged when it will be renamed to its stripped heading)
...
- **Piece 1 is the original.** The original fragment is truncated to the first piece's content, keeping its UUID, aspects, readiness, references, unmanaged frontmatter, and all sequence placements. Its **key** is untouched too — except when the heading is stripped (the default) and the original's body starts with a heading: then the original is renamed to that heading's derived key (the storage service cascades the file + Margin rename and rewrites `[[fragments/oldKey]]` links). Leading prose before the first heading has no heading to strip, so the original keeps its key. A collision on the derived key is suffixed like any other; the original's own old key is not a false collision (it is freed by the rename).
...
- After a split, the original fragment retains its UUID, aspects, readiness, references, and every sequence placement, with content equal to the first piece. It keeps its key too, unless the heading is stripped (the default) and its body starts with a heading — then it is renamed to that heading's derived key and `originalKeyRenamedTo` records the new key.
