...
- 2026-05-26 - Defined a command scope for the command palette itself, to ensure all hotkey binds are handled by the command system. Command palette actions are now visible within the command palette itself.
- 2026-06-08 — **Command failure observability**: commands declare `onFailure`; failures surface as a toast (friendly message + technical detail) and a `command:error` entry in the action log. Every log entry carries a required `correlationId` (server-generated, echoed via the `X-Correlation-Id` header); `executeCommand` records backend command failures, the frontend records pre-flight/network failures. History page renders failures as a distinct row with a details disclosure and a "Show errors" filter. `useCommandScope` gains an `onCommandError` filter for in-place handling. See `references/adr/0012-command-failure-observability.md`.
