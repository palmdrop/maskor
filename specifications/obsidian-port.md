...
1. **Aspect-arc model + overview graph** (`aspect-arc-model.md`, `overview.md`): weighted aspect dimensions per fragment, actual-arc curves plotted against sequence order. No ecosystem plugin plots a frontmatter property as a function of manuscript position. This is the product; it lives in `maskor-core` as a custom `ItemView`.
...
- **Preview surface** (`preview.md`): Longform Compile emits files; the live assembled read-only document (scroll-spy "you are here", shareable fragment anchors, overlay editing) has no community equivalent. Either a preview `ItemView` in `maskor-core` or an accepted loss.
