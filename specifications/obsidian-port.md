...
- Patching Obsidian's Canvas internals (Advanced Canvas-style monkey-patching). There is still no official Canvas runtime API (2026); maskor never depends on patched internals for core workflow. Consuming events _exposed by_ [Advanced Canvas](https://github.com/Developer-Mike/obsidian-advanced-canvas) when it happens to be installed is permitted garnish, nothing more.
