...
- 2026-07-10 — Document links polish (plan: `references/plans/document-links-polish.md`). Regression coverage pinning post-split link resolution (distinct active keys + active-preferring, order-independent key→uuid resolution). `[[` autocomplete now accepts with **Tab** in all three editor modes (TipTap popup + CM6, the latter gated on an active completion). A rich-mode toolbar **Insert link** button (opens the entity picker via the command palette). Document links in **Margin comment + notes editors** — autocomplete, resolved/broken styling, and click-to-navigate in the editors and in static comment/notes text — reusing the existing extension builders. Comment/notes bodies are link readers only (no backlinks / link-table sources — see Open questions).
