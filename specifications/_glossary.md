...
**Tile** (retired): The horizontal-grid representation of a single placed fragment in the legacy Overview, rendered with density-dependent content (key, excerpt, aspect chips, or color bar). The redesigned vertical Overview replaced tiles with per-fragment prose blocks and condensed title rows, and the last surviving tile component (`TileContent`, formerly used by the place-in-sequence modal) was removed once that modal moved to the shared `SequenceArranger` — so "tile" no longer exists in the codebase. Kept here as a retired term: do not reuse "tile" for the Overview's rows or blocks. _Avoid_: card (used generically elsewhere in the UI), cell, block.
...
**Command failure**: An unexpected termination of a user-initiated command at execution time — a thrown error that prevented the command from completing its intended state change. Distinct from a **Warning** (which is vault-level, advisory, and non-fatal) and from a **Command** being disabled (which is a valid pre-run guard, not a failure). Surfaced immediately to the user via a toast and recorded as a `command:error` entry in the action log. Each command failure is traceable via its **Correlation ID**. _Avoid_: error (too generic), warning (vault-level concept), exception.

**Correlation ID**: A UUID generated by the backend per API request (or by the frontend for frontend-only command failures), carried through response headers (`X-Correlation-Id`), all action log entries, and backend structured logs; enables tracing any user-initiated action — success or failure — across all layers, and identifying whether a command failure was already recorded by the backend. Required on all action log entries. _Avoid_: request ID (too HTTP-specific), trace ID (implies deeper tracing infrastructure), invocation ID.
