...
- 2026-06-18 — Dev DB auto-reset made discoverable: documented the opt-in `MASKOR_DB_AUTO_RESET` flag in `.env.example` (the feature shipped 2026-06-01 but was undocumented and there is no `.env`, so it never fired in dev — the "schema changes still cause permanent DB errors" symptom). Off by default; requires a real `.env`.
