...
The core idea is to have a database for quick lookups and queries, but keep all the core data related to a project human-readable. However, for now, sequences and interleaving are not represented as markdown files. This is for future development.

In the future, we might add support for a DB-only backend, but this is deferred for now.
...
- Sequences and sections — no vault representation for now; deferred
- Interleaving
...
| ------------------------- | --------------------- | --------------------------------------------------------------- |
| `uuid`                    | Vault (frontmatter)   | Assigned by Maskor on first detection; never changed after      |
| `title`                   | Vault (frontmatter)   | Display name; independent from the file name                    |
| `updatedAt`               | Vault (frontmatter)   | Written by Maskor on every API write; user should not edit      |
| `readyStatus`             | Vault (frontmatter)   | Float `0–1`; user-set or auto-generated by Maskor on save       |
| `notes[]`, `references[]` | Vault (frontmatter)   | Title/name arrays; relationship owned by the fragment or aspect |
| Inline aspect weights     | Vault (inline fields) | Dataview syntax: `aspect-key:: 0.8`                             |
| Body content              | Vault (body)          | Maskor never modifies                                           |
| `contentHash`             | DB only               | SHA of full file content (frontmatter + body) at last sync      |
| `syncedAt`                | DB only               | Unix ms; set by Maskor on each DB sync event                    |
| `filePath`                | DB only               | Relative to vault root; updated on file rename                  |
| `isDiscarded`             | DB only               | Derived from `filePath.startsWith("discarded/")` at index time  |
| Sequence positions        | DB only               | All ordering data — no vault representation                     |
| Fitting scores            | DB only               | Computed from aspects, arcs, and sequence context               |
| Arc positions             | DB only               | Where fragment sits on each arc at its sequence index           |
