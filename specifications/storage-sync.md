...
- 2026-07-04 — Swap contract: baseline fingerprint. A swap file (and `PUT /swap` body / `GET /swap` response) carries an optional `baseHash` — a client-computed fingerprint of the server content the buffered edits diverged from. Recovery uses it to distinguish a single-tab crash (baseline still matches the current server → auto-apply as before) from a stale multi-tab overwrite (the server advanced elsewhere → an explicit "Backup conflict" choice; see `specifications/fragment-editor.md`, Buffer authority). Additive and backwards-compatible: swaps written without it read back as `baseHash: null` and keep the pre-baseline behaviour. (plan: references/plans/multi-tab-swap-hardening.md)
- 2026-06-25 — Swap survival invariant locked in. `.maskor/swap/` (the transient unsaved-content crash net) is exempt from every DB reset (dev auto-reset and manual `index.reset`) and every draft snapshot/restore — those touch only `vault.db` (+ sidecars) and the explicit fragments/aspects/notes/references + `.maskor/{sequences,config,project.json,action-log.jsonl}` allowlist. Already true incidentally; now an explicit, regression-tested invariant so a future `.maskor/` teardown can't silently throw away an in-flight unsaved edit. (plan: references/plans/never-lose-writing.md, Phase 5)
- 2026-06-22 — Dev DB auto-reset no longer wipes telemetry on a normal new migration. The schema fingerprint is now **prefix-stable** (it folds each migration's tag + SQL body in apply order), so `classifySchemaState` distinguishes a _forward-only addition_ — the stamp matches a proper prefix of the current set — from _genuine drift_ (an amended or removed migration, or a corrupt file). A forward addition is left in place for `migrate()` to apply incrementally (preserving `fragment_stats` and the rest of the DB-only state) and then re-stamped; only drift triggers the drop+rebuild. Previously every `db:generate` + restart reset the whole DB because the reset ran before `migrate()` on any fingerprint change. The fingerprint is now re-stamped after a successful `migrate()` whenever the DB matches the current set (fresh, already-current, or forward), but never when a drift is left in place with the flag off (so the stale stamp keeps the drift detectable on a later flag-on run).
- 2026-06-18 — Swap list endpoint + unsaved-changes indicator: `GET /projects/:projectId/swap` returns every entity that currently has an unsaved-content swap file (`{ entries: [{ entityType, entityUUID, savedAt }] }`), backed by the existing `swap.list` storage primitive. The fragment list and the Overview reorder column surface an "unsaved changes" dot on fragments carrying a fragment swap (edits that survived navigation and were not yet saved/cleared). Read-only; the swap routes stay outside the commands pipeline and action log.
- 2026-06-18 — Dev DB auto-reset now fires out-of-the-box in dev: the API `dev` script sets `MASKOR_DB_AUTO_RESET=1` inline, so `bun run dev` auto-resets `vault.db`/`registry.db` on a migration-set change (eliminating the manual "delete the DB + restart" loop). The packaged `start` script never sets it, so production never resets. Root cause of the prior "still not working" report: the feature shipped 2026-06-01 but the flag was undocumented and there was no `.env`, so `isAutoResetEnabled()` was always false — the reset never ran. The flag is also documented in `.env.example` for non-`dev` runs. The reset fires on the next process restart after the migration set changes; under `bun --watch`, saving the imported `schema.ts` triggers that restart.
- 2026-06-16 — Document-link index. A `links` table holds every `[[type/key]]` edge parsed from fragment / note / reference bodies (forward edges: source type/uuid, target type/key, nullable resolved `target_uuid`, alias, ordinal, snippet). It is a derived index — vault files authoritative — maintained inline on every entity upsert (`syncLinks` replaces a source's rows; entity appearance/disappearance binds/un-binds rows) and rebuilt from scratch on `index.rebuild` (cleared, repopulated during upserts, then a final resolution pass). The watcher keeps it current between rebuilds; deletion clears the source's rows and un-binds rows pointing at the deleted entity (broken links persist). Inline `[[references/…]]` / `[[aspects/…]]` links also **auto-sync into fragment metadata** on a content save / external-edit cycle (canonical write-back). See `specifications/document-links.md`. (plan: `references/plans/document-links.md`)
...
- 2026-06-04 — `suggestion.currentFragmentUUID` moved from project manifest to vault DB. A new `project_state` table (single-row, `id = 1`) holds the pointer as DB-only state — it is not re-derivable from vault files. `getCurrent` reads and `getNext` writes the pointer from/to this table; the manifest no longer carries or rewrites `currentFragmentUUID` on every suggestion advance. (plan: `scripts/ralph/archive/2026-06-04-small-improvements/`)
...
- 2026-06-01 — Margin DB index, watcher sync & API. The vault DB carries a `margins` row + per-comment `comments` rows (`marker_id`, excerpt, body, `ordinal`); rebuild and the watcher keep them in sync. `margin:synced` / `margin:deleted` SSE events fire on change. API: read/replace a fragment's Margin. (Originally this entry also shipped an `orphaned` column derived at sync time, per-comment `create`/`update`/`delete` endpoints, and a `list orphaned comments` endpoint — all **removed 2026-06-04** (migration `20260604_drop_comment_orphaned`): the client replaces the whole Margin via the replace endpoint and derives orphan state live from the open fragment buffer, so nothing read the flag. `margin:synced` now fires from a fragment edit only when an anchored comment's stored excerpt actually changes. See `specifications/margins.md`.) (plan: references/plans/margins.md, Phase 2)
...
- **`.maskor/swap/` is exempt from every DB reset and draft restore.** The swap directory is the transient unsaved-content crash net (see `specifications/fragment-editor.md`). A vault-DB reset (dev auto-reset or manual `index.reset`) drops only `vault.db` + its `-wal`/`-shm` sidecars; a draft snapshot/restore touches only the explicit allowlist (`fragments/aspects/notes/references` + `.maskor/{sequences,config,vault.db,project.json,action-log.jsonl}`). Neither purges `swap/`, so an in-flight unsaved edit is never thrown away by a reset or restore. This is an explicit invariant, regression-tested — any future `.maskor/` teardown must preserve `swap/`.
...
- **DB-only state (not re-derivable)**: Three categories of state live only in the vault DB — no vault file carries them. `index.rebuild` preserves them; a full DB drop loses them. This is why the dev-only auto-reset (below) is opt-in and gated. (1) `fragment_stats` behavioral telemetry (`voluntaryOpenCount`, `promptAcceptCount`, `avoidanceCount`, `editCount`, `lastSurfacedAt`). (2) `UUID_COLLISION` event warnings (event warnings are never wiped by rebuild). (3) `project_state.currentFragmentUUID` — the current suggestion-mode pointer, stored in the `project_state` table (single-row, `id = 1`) rather than the project manifest so it does not churn `project.json` on every suggestion advance.
