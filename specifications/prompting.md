...
- 2026-06-04 — `editCount` now increments at most once per suggestion-mode visit (fragment surfaced → navigated away), not once per save. Multiple saves during a single session count as one edit. A save with no content change never increments `editCount`. The session boundary is the cooldown window: once `editedWhileSurfaced` is set for a surfacing, subsequent saves in that window are no-ops for the counter. (plan: `scripts/ralph/archive/2026-06-04-small-improvements/`)
