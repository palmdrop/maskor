...
- 2026-06-04 — Anchor deletion drops the anchor (margins-4): when an edit deletes the content around an anchor (rich: ProseMirror `mapResult(...).deleted`; vim/raw: CM6 `mapPos(..., MapMode.TrackDel)` → `null`), the anchor is dropped rather than collapsed onto the neighbouring block — so deleting a paragraph orphans its comment and pasting it back re-attaches by excerpt. (plan: references/plans/margins-4.md, Phase 7)
