...
- 2026-07-17 — **Sidebar sequence rows open the placement modal**: clicking a sequence in the metadata sidebar's "Sequences" list now opens the "Place in sequence" modal for that sequence (reusing the editor's existing `openPlaceInSequence`) instead of navigating to the Overview — quicker for arranging the open fragment. The modal gained an "Open in Overview" link (closing the modal) for the full view, and its rows now draw the spine's title-mode length bars (see `sequencer.md`).
- 2026-07-17 — **Sequence-position indicator in the metadata sidebar**: each entry in the sidebar's read-only "Sequences" list now carries a small track with a vertical tick showing roughly where the fragment sits in that sequence (leftmost = first, rightmost = last, centered when it is the only fragment). Index-based across all sections in section order — not proportional to fragment length. A `title`/aria label reports the exact "Position N of M". (`fragment-sequence-membership.tsx`)
- 2026-07-04 — **Multi-tab swap hardening — conflicting-backup recovery**: a swap backup now records the _baseline_ it diverged from (a fingerprint of the server content at write time). On reopen, a backup whose baseline no longer matches the current server content — the fragment was saved in another tab or edited externally since the backup was written — is **not** silently auto-applied (which previously reverted the newer work). Instead a "Backup conflict" banner requires an explicit choice: _Keep server version_ (revert + drop the backup) or _Restore backup_ (apply it, marking the buffer dirty). The fragment ↔ Margin pair conflicts and restores as one. Same-baseline recovery (the single-tab crash net) still auto-applies as before, and legacy baseline-less swaps keep the old behaviour. (plan: references/plans/multi-tab-swap-hardening.md)
- 2026-06-25 — **Never silently lose writing** (defense-in-depth for the save/swap pipeline): the editor's unsaved-edit protections (explicit save, swap mirror, buffer authority) no longer all hinge on a single change-notification event that could silently fail. (1) The content-load guards are crash-safe (`try/finally`), so a thrown `setContent`/anchor-load can't leave the editor permanently deaf to edits. (2) A low-frequency dirty backstop re-derives "the buffer differs from the server" independently of the change event and re-engages the protections if a change was ever missed. (3) When the swap (the only crash net for unsaved prose) fails to write, a persistent, non-dismissable "Unsaved changes are not being backed up — copy your work" banner appears (combined over the fragment ↔ Margin pair), cleared when a swap write next succeeds. (plan: references/plans/never-lose-writing.md, Phases 1–3)
...
**Baseline-aware swap recovery.** Every swap write records the _baseline_ — a fingerprint (`hashContent`, trailing-whitespace-tolerant to match the server's `body.trim()` normalization) of the server content the buffer diverged from. On reopen, recovery compares the stored baseline against the current server content:

- **Baseline matches** (single-tab crash / navigation): the backup is auto-applied and the standard recovery banner offers Restore-from-server — unchanged behaviour.
- **Baseline no longer matches** (the server advanced elsewhere — another tab saved, or an external edit — after the backup was written): the backup is a **conflict** and is never auto-applied; the buffer keeps the current server content and a "Backup conflict" banner requires an explicit choice — _Keep server version_ (revert both sides of the fragment ↔ Margin pair and drop the swaps) or _Restore backup_ (apply the backup, marking the buffer dirty so buffer authority protects it and save persists the user's chosen version). Either side of the linked pair conflicting makes the whole pair conflict; the pair always restores together.
- **No baseline** (legacy swap written before baselines existed): treated as non-conflicting — the old auto-apply behaviour.
