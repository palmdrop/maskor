...
- **Rename via Maskor**: Maskor atomically renames the vault file and updates all fragment frontmatter lists that referenced the old key to the new key. No orphan warnings. This is the expected path — renames through Maskor are safe. Case-only renames (e.g. `My Note` → `my note`) are handled via a temp-file roundtrip so no data is lost on case-insensitive filesystems (macOS APFS, Windows NTFS) — the same mechanism fragments use.
