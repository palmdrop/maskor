...
- Each comment **also stores a short excerpt** — the *opening* of the block it annotates (capped ~80 chars). The display excerpt is derived live from the marker's current block; the stored excerpt refreshes from the block's opening on fragment save and freezes once the comment is orphaned. It is used for side-by-side display and as orphan context; it is not the authoritative anchor.
...
- **Flow alignment by margin-side padding.** Comments shorter than their block are padded so rows line up; comments longer than their block are clipped (document-side push is deferred — see the deferred padding decision).
...
- **Notes** are a collapsible **pinned header** at the top of the column, scrolling with the content.
...
- **Guide line, no excerpt for anchored comments.** Anchored comments show a guide line to their paragraph and no excerpt; **orphaned** comments show their last-known excerpt in the orphan group at the foot.
...
- **The anchor marker must survive an editor round-trip in both modes.** In CM6/vim, a decoration always hides the marker (a line-end dot cue marks annotated lines; raw syntax only behind the "show source" toggle); in TipTap, the marker is a node attribute with a matching serializer. A naive marker (HTML comment, bare `^id`) would not survive TipTap's markdown→ProseMirror→markdown round-trip — a custom treatment in each editor is required.
