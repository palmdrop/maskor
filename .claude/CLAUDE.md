...
- To see the state of all in-flight work (plans × branches × worktrees × reviews × specs), run `bun run board` (or `bun run board --json` to consume it programmatically). It regenerates `references/STATUS.md`. The conventions are documented in `@references/WORKFLOW.md`.
