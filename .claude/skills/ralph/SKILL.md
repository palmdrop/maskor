...
Take a PRD (markdown file or text) and convert it to `prd.json` in your ralph directory (typically `scripts/ralph`).
...
```json
{
  ...
  "specs": ["specifications/spec1", "specifications/spec2"],
  "userStories": [
    {
      "id": "US-001",
      "title": "[Story title]",
...
7. **Specs**: Copy the paths under "Source Specifications" in the PRD's Introduction into the `specs` field verbatim. These are what Ralph will append `**Shipped**:` entries to on completion.
...
- [ ] `specs` field populated from the PRD's Source Specifications
