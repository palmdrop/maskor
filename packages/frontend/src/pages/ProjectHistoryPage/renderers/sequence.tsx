...
export const renderSequenceEntryText = (entry: ActionLogEntry): string => {
  switch (entry.type) {
    ...
    case "sequence:shuffled":
      return `Sequence "${sequenceName(entry)}" generated by shuffling ${entry.payload.fragmentCount} fragment${entry.payload.fragmentCount === 1 ? "" : "s"} (${entry.payload.constraintSequenceUuids.length} constraint${entry.payload.constraintSequenceUuids.length === 1 ? "" : "s"})`;
    default:
