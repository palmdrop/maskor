...
export const AdoptProjectDialog = ({ open, onOpenChange }: AdoptProjectDialogProps) => {
  ...
  const mutation = useMutation({
    mutationFn: (body: { name: string; vaultPath: string; mode: "adopt" }) =>
  ...
  const handleSubmit = () => {
    ...
    mutation.mutate({ name: nameInput.trim(), vaultPath: pickedPath, mode: "adopt" });
  };
  ...
  const entries = listQuery.data?.data.entries ?? [];
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {step === "picker" ? "Choose a folder to adopt" : "Confirm adoption"}
          </DialogTitle>
        </DialogHeader>

        {step === "picker" && <FolderPicker onSelect={handlePick} />}

        {step === "confirm" && pickedPath && (
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="adopt-project-name">Project name</Label>
              <Input
        ...
        {step === "confirm" && (
          <DialogFooter>
            ...
            >
              {mutation.isPending ? "Registering…" : "Adopt project"}
            </Button>
          </DialogFooter>
        )}
      </DialogContent>
