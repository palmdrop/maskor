...
import { usePersistedCursor } from "./usePersistedCursor";

describe("usePersistedCursor", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    localStorage.clear();
  });
  ...
  it("read returns null when nothing is stored", () => {
    const { result } = renderHook(() => usePersistedCursor("key-a"));
  ...
  it("persists the offset after the debounce delay", () => {
    ...
    expect(localStorage.getItem("key-a")).toBe("42");
  ...
  it("coalesces rapid saves into the last value", () => {
    ...
    act(() => {
      ...
      result.current.save(3);
      vi.advanceTimersByTime(200);
    });

    expect(localStorage.getItem("key-a")).toBe("3");
  });

  it("flushes a pending write on unmount", () => {
    const { result, unmount } = renderHook(() => usePersistedCursor("key-a"));
    ...
    unmount();
    expect(localStorage.getItem("key-a")).toBe("7");
  });

  it("persists a pending write to the key it was issued under after a key change", () => {
    const { result, rerender } = renderHook(({ storageKey }) => usePersistedCursor(storageKey), {
    ...
    expect(localStorage.getItem("key-a")).toBe("5");
