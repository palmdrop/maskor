...
7. **Out-and-back returns (revival).** Originally specified as soft-delete revival: an `add` for a UUID whose row was soft-deleted clears `deletedAt`, updates `filePath`, and emits `revived: true`. **Implemented divergence**: the codebase uses hard-delete for keyed entities, and adding `deletedAt` everywhere wasn't justified once we noticed identity is already preserved by the frontmatter-UUID + UUID-keyed upsert. The `revived: true` flag is still emitted, but via an in-memory `RecentlyDeletedTracker` populated when the rename-buffer expiry hard-deletes a row. See ADR-0002 for the full rationale and rejected alternative.
8. **Cross-entity-type returns** (e.g. `aspects/x.md` → `notes/x.md`) are not preserved as the same logical entity. The source row is hard-deleted; the destination entity is created with the UUID from frontmatter (because identity is anchored there). Fragment frontmatter referencing the source entity is left untouched and surfaces as a sync warning on rebuild.
