...
- **Opt-in via the split dialog.** A checkbox in `SplitFragmentDialog` ("Add pieces to a new sequence"), default **unchecked**, revealing an editable name input pre-filled with `<original key> split`. Non-empty name required to confirm when checked.
