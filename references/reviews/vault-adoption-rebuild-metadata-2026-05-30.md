...
- **New finding (data-loss risk, pre-existing)** — surfaced while testing the fix: `parseFile` returned gray-matter's cached, shared-mutable `.data`, and the adoption helpers mutate `frontmatter.uuid` in place, so two byte-identical files shared one frontmatter object. Adopting a vault with duplicate-content keyed entities could silently collapse them onto one UUID. **Fixed** — `parse.ts` now shallow-copies `.data` so each `ParsedFile` owns its frontmatter; `parse.test.ts` adds an "independent frontmatter object" regression test.
