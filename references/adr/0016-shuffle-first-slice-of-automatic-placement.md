...
- The generated sequence lands **inactive** (`active: false`). It totally orders every fragment, so if it were active it would join the active constraint set immediately and could contradict an active secondary the user left out of the chosen constraints — manufacturing a violation/cycle the moment it lands. Inactive-by-default makes a fresh shuffle a candidate the user activates deliberately.
...
- **Algorithm note (2026-07-16)**: the original whole-universe seeded Kahn's sampler skewed constrained fragments toward the end (a chain exposes one ready node at a time while every free fragment is ready immediately, so the free pool drained first). Replaced by uniform slot assignment: Fisher–Yates over the universe, then each weakly connected constraint component's members are rewritten into their occupied slots in topological order. Positions are uniform for every fragment; disjoint chains sample the uniform linear-extension distribution exactly.
